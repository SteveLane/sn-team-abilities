---
title: "Round 6, 2018"
subtitle: "Round 5 Wrapup and Round 6 Analysis"
---

<!-- Time-stamp: <2018-06-02 13:33:07 (slane)> -->
...
knitr::opts_chunk$set(cache = FALSE, error = FALSE, warning = FALSE,
                      ...
                      fig.asp = 9/16, cache.path = "round6-2018_cache/")
options(digits = 2)
...
if (!dir.exists(here("Rmd", "sn-assets", "round6-2018"))) {
    dir.create(here("Rmd", "sn-assets", "round6-2018"), recursive = TRUE)
}
...
First, let's download the round 5 data, and update the seasonal data:

```{r round5-download}
season_2018 <- updateData(5)
...
```{r round5-games}
results <- matchPredictions(5, 2018, match_results)
saveRDS(results, here("Rmd", "sn-assets", "round6-2018", "results_match.rds"))
...
Now match the data to team ID's and add **Round 6** for predictions:
...
saveRDS(round6_2018, here("Rmd", "sn-assets", "round6-2018", "game.rds"))
stan_data <- with(
    model_data, list(nteams = 8, ngames = nrow(model_data),
                     ...
                     ngames_pred = 4,
                     pred_home = round6_2018$homeSquad,
                     pred_away = round6_2018$awaySquad,
                     first_round = 0))
...
```{r histogram}
res <- sapply(1:4, predDiffHist, model = output, game_lookup = round6_2018)
## Create a 2x2 grid of the figures.
pl_grid <- plot_grid(res[[1, 1]], res[[1, 2]], res[[1, 3]], res[[1, 4]])
save_plot(here("figs", "round6-2018-plot-grid.png"),
...
saveRDS(res_table,
        here("Rmd", "sn-assets", "round6-2018", "predictions.rds"))
for (i in 1:4) {
    ggsave(here("Rmd", "sn-assets", "round6-2018", paste0("game-", i, ".png")),
...
pl_abilities <- ggplot(abilities, aes(x = Round, y = med, group = squadName)) +
    ...
    ylab("Ability") +
    xlim(c(0, 14))
ggsave(here("Rmd", "sn-assets", "round6-2018", "abilities.png"),
...
pl_hga <- ggplot(hga, aes(x = fct_reorder(squadName, med),
    ...
    coord_flip()
ggsave(here("Rmd", "sn-assets", "round6-2018", "hga.png"),
