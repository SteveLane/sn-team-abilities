---
title: "Round 8, 2018"
subtitle: "Round 7 Wrapup and Round 8 Analysis"
---

<!-- Time-stamp: <2018-06-22 21:14:40 (slane)> -->
...
knitr::opts_chunk$set(cache = FALSE, error = FALSE, warning = FALSE,
                      ...
                      fig.asp = 9/16, cache.path = "round8-2018_cache/")
options(digits = 2)
...
if (!dir.exists(here("Rmd", "sn-assets", "round8-2018"))) {
    dir.create(here("Rmd", "sn-assets", "round8-2018"), recursive = TRUE)
}
...
First, let's download the round 6 data, and update the seasonal data:

```{r round7-download}
season_2018 <- updateData(7)
...
match_results <- matchResults(season_2018) %>%
    filter(round == 7)
ladder <- ladders(season_2018, 7) %>%
    arrange(desc(points), desc(percentage)) %>%
    mutate(`Old Position` = 1:8) %>%
    rename(Team = squadName, Percentage = percentage,
           `Old Points` = points) %>%
    select(-games, -goals_for, -goals_against)
new_ladder <- data_frame(
    Team = sort(ladder$Team),
    `New Points` = c(2, 34, 24, 30, 32, 32, 30, 36),
    `Bonus Points` = c(2, 14, 16, 14, 12, 16, 18, 16)
)
ladder <- left_join(ladder, new_ladder) %>%
    arrange(desc(`New Points`), desc(Percentage)) %>%
    select(Team, `New Points`, `Bonus Points`, everything())
saveRDS(ladder, here("Rmd", "sn-assets", "round8-2018", "ladder.rds"))
...
```{r round7-games}
results <- matchPredictions(7, 2018, match_results)
saveRDS(results, here("Rmd", "sn-assets", "round8-2018", "results_match.rds"))
...
Now match the data to team ID's and add **Round 8** for predictions:
...
saveRDS(round8_2018, here("Rmd", "sn-assets", "round8-2018", "game.rds"))
stan_data <- with(
    model_data, list(nteams = 8, ngames = nrow(model_data),
                     ...
                     ngames_pred = 4,
                     pred_home = round8_2018$homeSquad,
                     pred_away = round8_2018$awaySquad,
                     first_round = 0))
...
```{r histogram}
res <- sapply(1:4, predDiffHist, model = output, game_lookup = round8_2018)
## Create a 2x2 grid of the figures.
pl_grid <- plot_grid(res[[1, 1]], res[[1, 2]], res[[1, 3]], res[[1, 4]])
save_plot(here("figs", "round8-2018-plot-grid.png"),
...
saveRDS(res_table,
        here("Rmd", "sn-assets", "round8-2018", "predictions.rds"))
for (i in 1:4) {
    ggsave(here("Rmd", "sn-assets", "round8-2018", paste0("game-", i, ".png")),
...
pl_abilities <- ggplot(abilities, aes(x = Round, y = med, group = squadName)) +
    ...
    ylab("Ability") +
    xlim(c(0, 14))
ggsave(here("Rmd", "sn-assets", "round8-2018", "abilities.png"),
...
pl_hga <- ggplot(hga, aes(x = fct_reorder(squadName, med),
    ...
    coord_flip()
ggsave(here("Rmd", "sn-assets", "round8-2018", "hga.png"),
