...
<!-- Time-stamp: <2018-04-22 19:25:30 (slane)> -->
...
```{r shrink-ability}
stan_shrink <- with(abilities,
                    list(nteams = nrow(abilities), ability = med,
                         ability_sd = sigma))
shrink <- sampling(
    shrink_model,
    data = stan_shrink,
...
new_sigma <- c(extract(shrink, "tau")$tau)
new_sigma <- median(new_sigma, 2, median)

```

We now bring in the data for the priors:
...
stan_data <- with(
    model_data, list(nteams = max(homeInt), ngames = nrow(model_data),
                     ...
                     init_ability = abilities_latest$med,
