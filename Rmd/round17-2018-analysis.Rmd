---
title: "Round 17, 2018"
subtitle: "Round 16 Wrapup and Round 17 Analysis"
---

<!-- Time-stamp: <2018-08-22 19:41:02 (slane)> -->
...
knitr::opts_chunk$set(cache = FALSE, error = FALSE, warning = FALSE,
                      ...
                      fig.asp = 9/16, cache.path = "round17-2018_cache/")
options(digits = 2)
...
if (!dir.exists(here("Rmd", "sn-assets", "round17-2018"))) {
    dir.create(here("Rmd", "sn-assets", "round17-2018"), recursive = TRUE)
}

```

```{r set-round}
## This is a new chunk which sets the current and previous rounds so that I
## don't need to use a search and replace strategy.
## Will need to adjust for the finals...
this_round <- 17
previous_round <- this_round - 1
finals_round <- 2

```

## Round `r previous_round` Wrap-up

First, let's download the round `r previous_round` data, and update the seasonal data:

```{r round-download}
season_2018 <- updateFinals(previous_round, "10394", finals_round)
...
```{r round-games}
results <- matchPredictions(previous_round, 2018, match_results)
dir_text <- paste0("round", this_round, "-2018")
saveRDS(results, here("Rmd", "sn-assets", dir_text, "results_match.rds"))

```

And output the ladder:

```{r ladder}
ladder <- ladders(season_2018) %>%
    select(-games, Squad = squadName, `Goals for` = goals_for,
           `Goals against` = goals_against, Percentage = percentage,
           Points = points_new, `Points (old)` = points)
saveRDS(ladder, here("Rmd", "sn-assets", dir_text, "ladder.rds"))

```

## Round `r this_round` Analysis
...
Now match the data to team ID's and add **Round `r this_round`** for predictions:
...
saveRDS(round_data, here("Rmd", "sn-assets", dir_text, "game.rds"))
stan_data <- with(
...
```{r histogram}
## Adjusted for finals predictions
res <- sapply(1, predDiffHist, model = output, game_lookup = round_data)
## Create a 2x1 grid of the figures.
pl_grid <- plot_grid(res[[1, 1]])
save_plot(here("figs", paste0(dir_text, "-plot-grid.png")),
          pl_grid, base_height = 35 / (1 + sqrt(5)), base_width = 17.5)
...
saveRDS(res_table,
        here("Rmd", "sn-assets", dir_text, "predictions.rds"))
for (i in 1) {
    ggsave(here("Rmd", "sn-assets", dir_text, paste0("game-", i, ".png")),
...
pl_abilities <- ggplot(abilities, aes(x = Round, y = med, group = squadName)) +
    ...
    ylab("Ability") +
    xlim(c(0, 14))
ggsave(here("Rmd", "sn-assets", dir_text, "abilities.png"),
...
pl_hga <- ggplot(hga, aes(x = fct_reorder(squadName, med),
    ...
    coord_flip()
ggsave(here("Rmd", "sn-assets", dir_text, "hga.png"),
